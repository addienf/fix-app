<?php
return [
    [
        'mainPart' => 'Cooling System',
        'parts' => [
            'Measure the current of cooling system (compresor)',
            'Pressure analyzer (10Psi - 15Psi)',
            'Normal 2,58 - 2,76A in 220-240V/50Hz',
            'Normal 2,34 - 2,39A in 203-230V/60Hz',
            'Normal 3,46 - 3,71A in 220-240V/50Hz',
            'Normal 3,4 - 3,56A in 203-230V/60Hz',
            'Check the freon leak with halogen leak detector',
            'Cleaning evaporator sample must be unloaded 50 minutes (racks from row number 1 & 2',
        ]
    ],
    [
        'mainPart' => 'Humidification System',
        'parts' => [
            'Measure the resistance of the water heater',
            'Water Heater  1000 W (46-53 Ohm)',
            'Water Heater  1350 W (33-40 Ohm)',
            'Clean the container of humidifier',
            'Clean Water heater surface',
            'Clean & Check Level switch sensor',
            'Clean the water inlet',
            'Take the photo of the water heater condition and resistance in Ampere Mete',
        ]
    ],
    [
        'mainPart' => 'Air Heating System',
        'parts' => [
            'Measure the resistance of the air heater ',
            'Air  Heater 1500 W (30-38 Ohm)',
            'Air  Heater 1000 W (46-53 Ohm)',
            'Air  Heater 1350 W (33-40 Ohm)',
            'Take the photo of the water heater condition and resistance in Ampere Mete',
        ]
    ],
    [
        'mainPart' => 'Water Feeding System',
        'parts' => [
            'Measure the feeding water conductivity (Standard: 20-100 µS)',
        ]
    ],
    [
        'mainPart' => 'Controlling  System',
        'parts' => [
            'Check the analogue output temperature function ',
            'Check the analogue output humidity function',
        ]
    ],
    [
        'mainPart' => 'Logging System',
        'parts' => [
            'Take the screenshot/photo of chamber is connected to wonderware ',
            'Report Data',
            'Trend Graph',
            'Event and Historical Alarm',
        ]
    ],
    [
        'mainPart' => 'Alarm System',
        'parts' => [
            'Check all the individual alarm by triggering each of them',
            'Safety device',
            'Low water protection',
            'Temperature out of range',
            'Humidity out of range',
            'Low-level water',
            'Check all the alarm lamp',
            'Check all the alarm buzzer',
            'Check the synchronisation of individual alarms & CAS',
        ]
    ],
    [
        'mainPart' => 'Interior TL Lamp',
        'parts' => [
            'Check the lamp functionality',
            'Check the lamp cover condition',
        ]
    ],
    [
        'mainPart' => 'Check with Thermal Camera ',
        'parts' => [
            'MCB Panel (temperature : 35-45 C)',
            'Contactor (temperature : 35-45 C)',
            'SSR (temperature : 35-45 C)',
        ]
    ],
];
