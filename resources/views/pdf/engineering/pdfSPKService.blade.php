...
@section('content')
    <div id="export-area" class="p-2 text-black bg-white">
        ...
        <div class="max-w-4xl p-6 mx-auto text-sm text-black bg-white">
            ...
            <div class="mb-3 space-y-2">
                <label><input type="checkbox" name="deskripsi_pekerjaan[]" value="service"
                        {{ in_array('service', $service->deskripsi_pekerjaan) ? 'checked' : '' }}> Service</label><br>
                <label><input type="checkbox" name="deskripsi_pekerjaan[]" value="maintenance"
                        {{ in_array('maintenance', $service->deskripsi_pekerjaan) ? 'checked' : '' }}>
                    Maintenance</label><br>
                <label><input type="checkbox" name="deskripsi_pekerjaan[]" value="lainya"
                        {{ in_array('lainya', $service->deskripsi_pekerjaan) ? 'checked' : '' }}> Lainnya</label>
            </div>
            ...
            <table class="w-full mb-4 text-center border border-collapse border-black">
                <thead>
                    <tr class="bg-gray-100">
                        ...
                        <th class="px-2 py-1 border border-black">Nama Teknisi</th>
                        <th class="px-2 py-1 border border-black">Jabatan</th>
                ...
                <tbody>
                    @foreach ($service->petugas as $index => $data)
                        <tr>
                            ...
                            <td class="px-4 py-2 border">{{ $data['nama_teknisi'] }}</td>
                            <td class="px-4 py-2 border">{{ $data['jabatan'] }}</td>
                            <td class="px-4 py-2 border"><img src="{{ asset('storage/' . $data['ttd']) }}"
                                    alt="Create Signature" class="w-50 h-50" /></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
            ...
            <ol class="pl-5 mb-4 list-decimal">
                <li class="mb-2">
                    ...
                    <span class="ml-2">
                        <input type="radio" name="selesai" value="Ya" {{ $kondisi === 'ya' ? 'checked' : '' }}> Ya
                        <input type="radio" name="selesai" value="Tidak" class="ml-4"
                            {{ $kondisi === 'tidak' ? 'checked' : '' }}> Tidak
                    </span>
                </li>
                <li>
                    Catatan Tambahan:
            ...
            <div class="grid grid-cols-2 gap-2 text-center">
                ...
                <div>
                    ...
                    <div class="flex items-center justify-center w-40 h-24 mx-auto mb-2 bg-white border border-gray-400">
                    ...
                    <p class="font-semibold underline">{{ $service->pic->dikonfirmasi_nama }}</p>
                </div>
                <div>
                    ...
                    <p class="font-semibold underline">{{ $service->pic->diketahui_nama }}</p>
                </div>
            </div>
        </div>
    </div>
