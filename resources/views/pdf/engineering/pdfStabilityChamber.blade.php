...
@section('content')
    ...
    @php
        $rawDetails = $stabilityChamber->detail->checklist ?? [];
    ...
    <p style="text-align:center; font-weight:bold; margin-bottom:16px;">
        CTC Name / Tag No : {{ $stabilityChamber->tag_no }}
    </p>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="sign-cell">
                    <div class="sign-box">
                        <img src="{{ public_path('storage/' . $stabilityChamber->pic->checked_signature) }}">
                    </div>
                </td>
                <td class="sign-cell">
                    <div class="sign-box">
                        @if ($stabilityChamber->pic?->approved_signature)
                            <img src="{{ public_path('storage/' . $stabilityChamber->pic->approved_signature) }}">
                        @endif
                    </div>
                </td>
            </tr>
            <tr>
                ...
                <td>{{ \Carbon\Carbon::parse($stabilityChamber->pic->checked_date)->translatedFormat('d F Y') }}</td>
                <td>
                    @if ($stabilityChamber->pic?->approved_date)
                        {{ \Carbon\Carbon::parse($stabilityChamber->pic->approved_date)->translatedFormat('d F Y') }}
                    @endif
                </td>
            </tr>
        </tbody>
