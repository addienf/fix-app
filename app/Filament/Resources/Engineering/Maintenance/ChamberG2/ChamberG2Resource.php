...
use App\Filament\Resources\Engineering\Maintenance\ChamberG2\ChamberG2Resource\RelationManagers;
...
class ChamberG2Resource extends Resource
{
    ...
    {
        $defaultParts = collect(config('chamberG2'))
            ->map(function ($group) {
                return [
    ...
    protected static function signatureInput(string $fieldName, string $labelName): SignaturePad
    {
        return
            ...
            ->afterStateUpdated(function ($state, $set) use ($fieldName) {
                if (blank($state))
                    return;
                $path = SignatureUploader::handle($state, 'ttd_', 'Engineering/Maintenance/ChamberG2/Signature');
                if ($path) {
                    $set($fieldName, $path);
                }
            });
